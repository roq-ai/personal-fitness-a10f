...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createClient() {
    ...
    const body = { ...req.body };
    if (body?.credit_transaction?.length > 0) {
      const create_credit_transaction = body.credit_transaction;
      body.credit_transaction = {
        create: create_credit_transaction,
      };
    } else {
      delete body.credit_transaction;
    }
